...
class NotebooksCampinasController {
  async findAll(req: Request, res: Response) {
    ...
    return notebooks.length > 0 ? res.status(200).json(notebooks) : res.status(204).json({});
  ...
  async create(req: Request, res: Response) {
    const { data, marca, modelo, mac, serial, patrimonio, userCampinasId } = req.body;
  ...
  async update(req: Request, res: Response) {
    ...
    return notebooks ? res.status(200).json(notebooks) : res.status(204).json({});
  }
